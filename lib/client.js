...
const exchangesS = ['binance', 'coinbase', 'bybit-spot', 'okex']
...
function csvLinesToObjs(lines) {
  ...
  for (let line of lines) {
    ...
    for (let c = 0; c < cols.length; c++) {
      if (vals[c] === 'true') {
        vals[c] = true
      } else if (vals[c] === 'false') {
        vals[c] = false
      } else if (c >= nums) {
        vals[c] = parseFloat(vals[c])
      }
      obj[cols[c]] = vals[c]
    }
...
Client.prototype.__products = async function(type, delisted=false, retry=0) {
  ...
  try {
    ...
    const res = await request(path, { delisted }, this.key)
    return res

  } catch (err) {
    if (err.message.includes('429')) { return this.__products(type, delisted, retry + 1) }
    if (err.message.includes('500') && retry < this.retry) { return this.__products(type, delisted, retry + 1) }
    throw err
  }
}

Client.prototype.futures = function(delisted=false) {
  return this.__products(1, delisted)
}

Client.prototype.spot = function(delisted=false) {
  return this.__products(2, delisted)
}
...
function paramsToDepthSteps(params) {
  const res = []
  let reso = parseInt(params.resolution)
  let begin = parseInt(params.begin)
  let end = parseInt(params.end)
  ...
  if (resolutions.indexOf(reso) < 0 && (reso % 60) !== 0) { throw new Error('invalid resolution') }
  if (coin && (exchange || product)) { throw new Error('exchange and product params not allowed with coin param') }
  if (!coin && (!exchange || !product)) { throw new Error('need exchange and product params or coin param only') }

  let step = { reso, begin, end, forward: 1 }
  if (coin) {
    step.coin = coin
  } else {
  ...
  const minutes = end.diff(begin, 'minutes').minutes
  const count = minutes / reso
  const maxCount = 512

  if (count <= maxCount) { return [step] }

  step.end = step.begin + ((60 * 1000 * reso) * maxCount)
  res.push(step)

  while (step.end < end.ts) {
    step = Object.assign({}, step)
    step.begin = step.end
    step.end = step.begin + ((60 * 1000 * reso) * maxCount)
    step.end = Math.min(step.end, end.ts)
    res.push(step)
  }
...
Client.prototype.depth = function(params) {
  const key = this.key
  const path = '/api/v1/heatmap'

  async function depth(params, retry=0) {
    if (retry > 0) { await sleep(2000) }
    try {
      ...
      if (err.message.includes('429')) { return depth(params, retry + 1) }
      if (err.message.includes('500') && retry < this.retry) { return depth(params, retry + 1) }
      throw err
    }
  }

  params = alignResolution(params)
  const steps = paramsToDepthSteps(params)
  return iterator(steps, depth, true)
}
